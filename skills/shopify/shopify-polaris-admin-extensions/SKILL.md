...
description: "Add custom actions and blocks from your app at contextually relevant spots throughout the Shopify Admin via UI extensions. Use ONLY when working on files under an `extensions/` directory using `@shopify/ui-extensions-react`, scaffolding new admin extensions, action menu items, admin blocks, or admin print actions. UI extensions intentionally use Polaris web components (`s-*` tags). Do NOT use this skill for the main Remix app shell under `app/` — that uses Polaris React; see `shopify-polaris-react`."
